...
if exist ".\%~nx0" (
    ...
    set ide=e2
    set pj=..\
    rem the following is GNUC or LLVM or CCRL
    set tc=GNUC
) else (
    echo\
...
if %ide%==e2 if %tc%==LLVM set TOOLCHAIN_TOOLS_CHECK=make.exe clang.exe rl78-elf-objcopy.exe
...
if not %tc%==CCRL if not exist "%pj%src\smc_gen" if exist "%pj%src\"  (
    echo\
