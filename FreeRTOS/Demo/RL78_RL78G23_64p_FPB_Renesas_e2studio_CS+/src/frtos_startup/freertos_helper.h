...
#define R_CG_FREERTOS_INTERRUPT_EI(function) \
    function(void) __attribute__((section(".lowtext." #function), interrupt)); \
    static void _##function(void) __attribute__((section(".lowtext." "_" #function))); \
    void function(void) \
    { \
        /* This enables nested interrupts and doesn't return here. */ \
        portCALL_ISR_C_HANDLER_USING_FREERTOS_API(_##function); \
...
#if 1 /* Type of functio attribute: "#if 1" is naked, "#if 0" is interrupt. */

#define R_CG_FREERTOS_INTERRUPT_EI(function) \
    function(void) __attribute__((section(".lowtext." #function), naked)); \
    static void _##function(void) __attribute__((section(".lowtext." "_" #function))); \
    void function(void) \
    { \
        /* This enables nested interrupts and doesn't return here. */ \
...
#define R_CG_INTERRUPT_EI(function) \
    ...
    { \
        ...
        portASM("  MOV CS, #0  "); \
        portASM("  MOVW BC, %0  " portCDT_NO_PARSE( :: ) "g" ( _##function )  ); \
        /* _vPortCall_ISR_C_Handler_Without_Using_FreeRTOS_API() enables nested interrupts. */ \
        /* The following code is intended to remove the epilog code. */ \
        while (1) {} \
    } \
    static void _##function
